...
def execute_cmd(cmd, cmd1):
    if cmd == 'ctime':
        ...
        # рассказать анекдот
        speak("Зима, стоят мужики на остановке. Один достаёт сигарету и грит мол Огоньку не найдётся? Второй достаёт зажигалку, бензиновую, а она протекла по всей куртке. Чиркает, загорается вся рука. Ну а первый закуривает прям от руки: Ну, ты бля, фокусник!")
        ...
        #print("text: ", cmd1)
        f = open('C:/Users/User/Documents/python/djarvis/example.txt','a', encoding='utf-8')
        f.write(cmd1 + '\n') 
        f.close()
        speak("Записал")
    elif cmd == 'text_read':
        #for x in opts["cmds"]["text_read"]:
         #   cmd1 = cmd1.replace(x, "").strip()
        f = open('C:/Users/User/Documents/python/djarvis/example.txt','r', encoding='utf-8')
        print(f.read()) 
        speak("Печатаю")
        #print(f.read()) 
        f.close()
